...
{
    public abstract class WorkflowTestBase : CommonTestBase
    {
        protected IServiceProvider ServiceProvider;
        private static ILogger _logger;
		
        public WorkflowTestBase(CommonFixture data)
            : base(data)
        {
            _logger = LoggerFactory.CreateLogger(nameof(WorkflowTestBase));
        ...
        protected static async Task<string> GetAccessTokenAsync(string clientId, string clientSecret, string clientScope)
        {
            ...
            if (string.IsNullOrEmpty(tokenEndpoint))
            {
                ...
                else
                {
                    ...
                    tokenEndpoint = disco.TokenEndpoint;
#else
                    //var disco = new DiscoveryClient(GlobalConfig.SSOWebDomain);
