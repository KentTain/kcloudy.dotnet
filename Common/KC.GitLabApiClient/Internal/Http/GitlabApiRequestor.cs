...
namespace KC.GitLabApiClient.Internal.Http
{
    internal sealed class GitLabApiRequestor
    {
        ...
        public async Task<T> Post<T>(string url, IEnumerable<KeyValuePair<string, string>> keyValues)
        {
            ...
            var responseMessage = await _client.PostAsync(url, postContent);
            await EnsureSuccessStatusCode(responseMessage);
            return await ReadResponse<T>(responseMessage);
        }

        public async Task Post(string url, IEnumerable<KeyValuePair<string, string>> keyValues)
        {
            var postContent = new MultipartFormDataContent();
            postContent.Headers.Add("ContentType", $"multipart/form-data");
            if (keyValues != null)
