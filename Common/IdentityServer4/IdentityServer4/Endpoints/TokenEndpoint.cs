...
namespace KC.IdentityServer4.Endpoints
{
    ...
    /// The token endpoint
    /// </summary>
    /// <seealso cref="KC.IdentityServer4.Hosting.IEndpointHandler" />
    internal class TokenEndpoint : IEndpointHandler
    {
        ...
        public async Task<IEndpointResult> ProcessAsync(HttpContext context)
        {
            ...
            {
                var msg = !HttpMethods.IsPost(context.Request.Method)
                    ? "the HTTP request method must be POST"
                    : !context.Request.HasFormContentType
                        ? "the Content-Type header need to represent a form content type"
                        : "Invalid HTTP request for token endpoint";
                _logger.LogWarning(msg);
                return Error(OidcConstants.TokenErrors.InvalidRequest, msg);
            }
