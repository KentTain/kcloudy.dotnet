...
namespace KC.Service.DTO.Admin
{
    ...
    [ProtoContract]
    public class StoragePoolDTO : EntityDTO
    {
        [DataMember]
        [ProtoMember(1)]
        ...
        public string StorageTypeString
        {
            get
            {
                return StorageType.ToDescription();
            }
        }
        ...
        public string GetStorageConnectionString(string privateKey)
        {
            if (string.IsNullOrEmpty(AccessKeyPasswordHash) || string.IsNullOrEmpty(AccessName))
                return KC.Framework.Base.GlobalConfig.StorageConnectionString;

            const string conn = @"BlobEndpoint={0};DefaultEndpointsProtocol=https;AccountName={1};AccountKey={2}";
