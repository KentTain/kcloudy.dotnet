...
namespace KC.Service.DTO.Admin
{
    ...
    public class VodPoolDTO : EntityDTO
    {
        public VodPoolDTO()
        {
            CanEdit = true;
        ...
        [DataMember]
        public string VodTypeString
        {
            get
            {
                return VodType.ToDescription();
            }
        }
        [XmlIgnore]
        //[JsonIgnore]
        ...
        public string GetVodConnectionString(string privateKey)
        {
            if (string.IsNullOrEmpty(AccessKeyPasswordHash) || string.IsNullOrEmpty(AccessName))
                return KC.Framework.Base.GlobalConfig.VodConnectionString;
