...
namespace  Com.Common.FileHelper
{
    public class FileOperate
    {
        ...
        *****************************************/
        ...
        public static void WriteFile(string path, string strings, System.Text.Encoding encoding = null)
        {
            using (var fsNew = new FileStream(path, FileMode.OpenOrCreate, FileAccess.Write))
            {
                if (encoding == null)
                {
                    var bytes = System.Text.Encoding.Default.GetBytes(strings);
                    fsNew.Write(bytes, 0, bytes.Length);
                }
                else
                {
                    var bytes = encoding.GetBytes(strings);
                    fsNew.Write(bytes, 0, bytes.Length);
                }
            }
        }
