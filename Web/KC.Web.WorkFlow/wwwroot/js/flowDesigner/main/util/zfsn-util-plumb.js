    ...
    getInstance: function () {
        ...
        _base.plumb.bind('beforeDrop', function (info) {
            ...
            if (_base.graph.hasEdge(sourceId, targetId)) {
                layer.tips(CONFIG.msg.repeatRouter, $.string.getJQSel(sourceId), {
                    tips: [2, '#23262e'],
                    time: 1000
                });
                return false;
            ...
            _base.plumb.draggable(nodeId, {
                ...
                drag: function (event) {
                    ...
                    if (!_base.selectedMultipleFlag) {
                        layer.tips('X: ' + parseInt($('#' + event.el.id).offset().left - canvasX + scrollX) + '  Y: ' + parseInt($('#' + event.el.id).offset().top - canvasY + scrollY), $.string.getJQSel(event.el.id), {
                            tips: [1, '#23262e'],
                            time: 2000
                        });
                    }
