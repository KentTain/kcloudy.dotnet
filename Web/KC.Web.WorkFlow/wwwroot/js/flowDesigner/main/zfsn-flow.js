window.FLOW = {
	...
	registerNodeEvent: function(tempId) {
		...
		$(selector).mousedown(function(event) {
			...
			if (!_base.selectedMultipleFlag && !_base.allowMultipleSelectedFlag) {
				...
				//	time: 2000
				//});
				$(ZFSN.getJQSel($(this).attr('id'))).tooltip({
					position: 'right',
					content: _base.graph.node($(this).attr('id')).text,
					onShow: function () {
	...
	clearCanvas: function() {
		...
		$.messager.confirm("系统提示", "是否推荐该产品至主页？",
			function (r) {
				if (r) {
					// 1、保存状态为未保存
					$("#saveStatus").css('display', '');
	...
	changeGrid: function() {
		...
		$('#showGridId').tooltip({
			position: 'right',
			content: $('#showGridId').children(':last-child').text(),
			onShow: function () {
				$(this).tooltip('tip').css({
					backgroundColor: '#ddd',
					borderColor: '#ddd',
