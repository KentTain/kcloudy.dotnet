...
(function (window, $, undefined) {
    ...
    coreString.isDate = function (str) {
        var isTrue = coreString.isLongDate(str) || coreString.isShortDate(str);
    ...
    coreString.isZipCode = function (str) {
        addressCode = coreString.isNullOrEmpty(str) ? "" : String(str);
        let provinceAndCites = {
        ...
        if (provinceAndCites[parseInt(addressCode.substring(0, 2))]) {
            return true;
        } else {
            return false;
        }
    ...
    //  判断当前 String 对象是否是正确的 统一社会信用编码
    coreString.isSocialCreditCode = function (str) {
        let creditCode = coreString.isNullOrEmpty(str) ? "" : String(str);
        let patrn = /^[0-9A-Z]+$/;
        //18位校验及大写校验
        if ((creditCode.length !== 18) || (patrn.test(creditCode) === false)) {
            return false;
        } else {
            let aCode;//信用代码/税号的每一个值
            let aCodeValue;//信用代码/税号每一个值的权重
            let total = 0;
            let weightedFactors = [1, 3, 9, 27, 19, 26, 16, 17, 20, 29, 25, 13, 8, 24, 10, 30, 28];//加权因子
            let str = '0123456789ABCDEFGHJKLMNPQRTUWXY';
            //不用I、O、S、V、Z
            for (let i = 0; i < creditCode.length - 1; i++) {
                aCode = creditCode.substring(i, i + 1);
                aCodeValue = str.indexOf(aCode);
                total = total + aCodeValue * weightedFactors[i];
                //权重与加权因子相乘之和
            }
            ...
            let checkCode = creditCode.substring(17, 18);
            return logicCheckCode === checkCode;
        }
    };
    coreString.prototype.isSocialCreditCode = function () { return coreString.isSocialCreditCode(this); };
    ...
    //  判断当前 String 对象是否是正确的 营业执照
    coreString.isBusinessCode = function (str) {
        let busCode = coreString.isNullOrEmpty(str) ? "" : String(str);
        let ret = false;
    ...
    coreString.prototype.isBusinessCode = function () { return coreString.isBusinessCode(this); };

    //  判断当前 String 对象是否是正确的 组织机构代码
    coreString.isOrganizationCode = function (str) {
        let orgCode = coreString.isNullOrEmpty(str) ? "" : String(str);
        if (orgCode !== "") {
            let part1 = orgCode.substring(0, 8);
            let part2 = orgCode.substring(orgCode.length - 1, 1);
    ...
    coreString.prototype.isOrganizationCode = function () { return coreString.isOrganizationCode(this); };

    //  判断当前 String 对象是否是正确的 纳税人识别号
    coreString.isTaxpayerCode = function (str) {
        let taxpayerId = coreString.isNullOrEmpty(str) ? "" : String(str);
        if (taxpayerId !== "" && taxpayerId.length === 15) {
            let addressCode = taxpayerId.substring(0, 6);
            // 校验地址码
            let check = coreString.isZipCode(addressCode);
            if (!check) {
                return false;
            }
            // 校验组织机构代码
            let orgCode = taxpayerId.substring(6, 9);
            check = coreString.isOrganizationCode(orgCode);
            if (!check) {
                return false;
            }
            return true;
        } else if (taxpayerId !== "" && taxpayerId.length === 18) {
    ...
    coreString.prototype.isTaxpayerCode = function () { return coreString.isTaxpayerCode(this); };
    ...
    //  对字符串进行Html反编码。
    coreString.toHtmlDecode = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        let arrEntities = { 'lt': '<', 'gt': '>', 'nbsp': ' ', 'amp': '&', 'quot': '"' };
        return str.replace(/&(lt|gt|nbsp|amp|quot);/ig, function (all, t) { return arrEntities[t]; });
    };
    coreString.prototype.toHtmlDecode = function () { return coreString.toHtmlDecode(this); };
    ...
    //  转换成日期格式：yyyy-MM-dd。
    coreString.toShortDateString = function (str) {
        if (!coreString.isDate(str)) return str;
        let dateValue = coreString.toDate(str);
        if (dateValue == null) return str;
        return dateValue.format('yyyy-MM-dd')
    };
    coreString.prototype.toShortDateString = function () { return coreString.toShortDateString(this); };

    //  转换成日期格式：yyyy-MM-dd HH:mm:ss。
    coreString.toLongDateString = function (str) {
        if (!coreString.isDate(str)) return '';
        let dateValue = coreString.toDate(str);
        if (dateValue == null) return str;
        return dateValue.format('yyyy-MM-dd HH:mm:ss')
    };
    coreString.prototype.toLongDateString = function () { return coreString.toLongDateString(this); };
    ...
    //  将字符串对象转换成 金额（例如：12,345.676）
    //  str:传入的float数字 ，n:希望返回小数点几位
    coreString.toMoney = function (str, n) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        str = coreString.trim(str);

        n = n > 0 && n <= 20 ? n : 2;
        let s = coreString.toFloat(str).toFixed(n) + "";
    ...
    coreString.prototype.toMoney = function () { return coreString.toMoney(this); };

    //  下划线（例如：user_name）转驼峰 userName
    coreString.toCamelCase = function (str, isUpperFirstCase) {
        if (coreString.isNullOrEmpty(str)) return "";
        str = coreString.trim(String(str));

        str = str.replace(/\_(\w)/g, function (all, letter) {
            return letter.toUpperCase();
        });
        if (isUpperFirstCase)
            return coreString.toUpperFirstCase(str);
        return str;
    };
    coreString.prototype.toCamelCase = function (isUpperFirstCase) { return coreString.toCamelCase(this, isUpperFirstCase); };

    //  驼峰（例如：userNamee）转下划线 user_name
    coreString.toUnderlineCase = function (str, isLowerCase) {
        if (coreString.isNullOrEmpty(str)) return "";
        str = coreString.trim(String(str));
    ...
    coreString.prototype.toUnderlineCase = function () { return coreString.toUnderlineCase(this); };

    //  ⾸字母⼤写（例如：userName）转 UserName
    coreString.toUpperFirstCase = function (str) {
        if (coreString.isNullOrEmpty(str)) return "";
        str = coreString.trim(String(str));
    ...
    coreString.prototype.toUpperFirstCase = function () { return coreString.toUpperFirstCase(this); };

    //  ⾸字母小写（例如：UserName）转 userName
    coreString.toLowerFirstCase = function (str) {
        if (coreString.isNullOrEmpty(str)) return "";
        str = coreString.trim(String(str));
    ...
    coreString.prototype.toLowerFirstCase = function () { return coreString.toLowerFirstCase(this); };


    //  将金额（例如：12,345.676）转换成 数值
    coreString.fromMoney = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        str = str.replace(/[^\d\.-]/g, "");
        return coreString.toFloat(str);
    };
    coreString.prototype.fromMoney = function () { return coreString.fromMoney(this); };
    ...
    //  通过id获取jQuery选择器
    coreString.getJQSel = function (id) {
        if (id.indexOf('#') != 0) {
            id = '#' + id;
        }
        return id;
    };
    coreString.prototype.getJQSel = function () { return coreString.getJQSel(this); };

    // 获取移除了jQuery选择器前缀的id
    coreString.removeJQSel = function (id) {
        if (id.indexOf('#') == 0) {
            id = id.substring(1);
        }
        return id;
    };
    coreString.prototype.removeJQSel = function () { return coreString.removeJQSel(this); };

    //  通过class获取jQuery选择器
    coreString.getJQClassSel = function (id) {
        if (id.indexOf('.') != 0) {
            id = '.' + id;
        }
        return id;
    };
    coreString.prototype.getJQClassSel = function () { return coreString.getJQClassSel(this); };

    // 获取移除了jQuery选择器前缀的class
    coreString.removeJQClassSel = function (id) {
        if (id.indexOf('.') == 0) {
            id = id.substring(1);
        }
        return id;
    };
    coreString.prototype.removeJQClassSel = function () { return coreString.removeJQClassSel(this); };


    //  将字符串aaa,bbb,ccc转换为列表项 string[]{aaa,bbb,ccc}
    coreString.arrayFromCommaDelimitedStrings = function (str) {
        if (coreString.isNullOrEmpty(str)) return [];
        str = String(str);
        return str.split(',');
    };
    coreString.prototype.arrayFromCommaDelimitedStrings = function () { return coreString.arrayFromCommaDelimitedStrings(this); };
    ...
    //  选取某个对象的某个字段，将列表该对象的这个字段转换成：xxxx,yyy,zzz；该函数定义如下参数：
    //      array:  必需。 一个数组对象。
    //      mapFn: 必须。 一个 function 类型值，表示数组对象的某个属性；
    //  返回值：返回指定的数组对象 array 的后面长度为 length 的元素所构成的一个新的数组。
    //      如果 array 的值为 空，则返回一个空字符串；
    //      如果 array 的值大于0，则返回字符串：xxxx,yyy,zzz；
    coreArray.toCommaSeparatedStringByFilter = function (array, mapFn) {
        array = coreArray.likeArray(array) ? array : [];
        if (array.length == 0) { return ''; }
        if (!coreUtil.isFunction(mapFn)) { throw "传入的参数 mapFn 不是一个函数。"; }

        return array.map(mapFn).join(',')
    };
    coreArray.prototype.toCommaSeparatedStringByFilter = function (mapFn) { return coreArray.toCommaSeparatedStringByFilter(this, mapFn); };
    ...
    //      uniqueID: 可选，表示要设置为的 uniqueID 属性值；如果不定义该值，则默认用 coreUtil.CommonUtil.getGuid() 为其创建一个随机值。
    ...
    //  将对象以Post的方式跳转值另一个页面
    coreJquery.redirectPost = function (location, args) {
        var form = $('<form></form>');
        form.attr("method", "post");
        form.attr("action", location);

        $.each(args, function (key, value) {
            if (coreUtil.isArray(value)) {
                //循环第一层列表对象
                $.each(value, function (key1, value1) {
                    //循环第一层列表对象中的单个对象的属性
                    $.each(value1, function (key2, value2) {
                        if (coreUtil.isArray(value2)) {
                            //循环第二层列表对象
                            $.each(value2, function (key3, value3) {
                                //循环第二层列表对象中的单个对象的属性
                                $.each(value3, function (k, v) {
                                    var field = $('<input></input>');

                                    field.attr("type", "hidden");
                                    field.attr("name", key + "[" + key1 + "][" + key3 + "]." + k);
                                    field.attr("value", v);
                            ...
                            field.attr("type", "hidden");
                            field.attr("name", key + "[" + key1 + "]." + key2);
                            field.attr("value", value2);
    ...
    coreJquery.prototype.redirectPost = function (location, args) {
        coreJquery.redirectPost(location, args);
    };
