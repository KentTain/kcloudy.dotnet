...
{
    public class ConfigEntityRepository : EFRepositoryBase<ConfigEntity>, IConfigEntityRepository
    {
        public ConfigEntityRepository(EFUnitOfWorkContextBase unitOfWork)
            : base(unitOfWork)
        {
        }
        ...
        {
            var dbContext = EFContext.Context as ComConfigContext;
...
";
        ...
        public ConfigEntity GetConfigWithAttributesById(int id)
        {
            return Entities.AsNoTracking()
                .Include(m => m.ConfigAttributes)
                .FirstOrDefault(c => c.ConfigId == id && !c.IsDeleted);
        }
        public ConfigEntity GetConfigWithAttributesByName(string configName)
        {
            return Entities.AsNoTracking()
                .Include(m => m.ConfigAttributes)
                .FirstOrDefault(c => c.ConfigName == configName && !c.IsDeleted);
        }

        public List<ConfigEntity> GetConfigsWithAttributesByFilter(Expression<Func<ConfigEntity, bool>> predicate)
        {
            var result = Entities
                .Include(o => o.ConfigAttributes)
                .AsNoTracking()
                .Where(predicate)
                .ToList();
            return result;
