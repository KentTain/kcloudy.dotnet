...
namespace KC.DataAccess.Config.DataInitial
{
    public static class DBSqlInitializer
    {
        ...
        //}
        public static List<string> lines = EmbeddedFileHelper.GetEmbeddedFileContents("KC.DataAccess.Config.DataInitial.CurrentVersion.txt");
        public static string GetPreMigrationVersion()
        {
            //List<string> lines = EmbeddedFileHelper.GetEmbeddedFileContents("KC.DataAccess.Config.DataInitial.CurrentVersion.txt");
            return lines[0].Trim();
        }
        ...
        {
            var key = "KC.DataAccess.Config.DataInitial.SchemaUpgrade.SQL_" + version + ".sql";
        ...
        {
            var key = "KC.DataAccess.Config.DataInitial.SQL_Structure.sql";
            var cache = GetCache(key);
            if (cache == null)
            {
                var sqls = EmbeddedFileHelper.GetEmbeddedFileContents(key);
        ...
        {
            var key = "KC.DataAccess.Config.DataInitial.SQL_InitialData.sql";
            var cache = GetCache(key);
            if (cache == null)
            {
                var sqls = EmbeddedFileHelper.GetEmbeddedFileContents(key);
