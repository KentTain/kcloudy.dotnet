...
begin	
	...
	if (@CurentVal  is null)
		begin
			...
			begin
				--select @RegularVal = @currdate + right(replicate(0,@length) + '1', @length) 
				select SeedType = @seqname, SeedValue = @PreFixString + @datetime + right(replicate(0,@length) + '1', @length), SeedMin = 1, SeedMax = @StepValue
			end
			else
			begin
				--select @RegularVal = @currdate + right(replicate(0,@length) + '1', @length) 
				select SeedType = @seqname, SeedValue = @PreFixString + @datetime + right(replicate(0,@length) + '1', @length) + @PostFixString, SeedMin = 1, SeedMax = @StepValue
			end
			commit tran T1
		end
	else
		begin
			...
			if(@PostFixString is null)
			begin
				--select @RegularVal = @currdate + right(replicate(0,@length) + cast((@CurentVal + @StepValue) as varchar), @length) 
				select SeedType = @seqname, SeedValue = @PreFixString + @datetime + right(replicate(0,@length) + cast((@CurentVal + @StepValue) as varchar), @length), SeedMin = @CurentVal + 1, SeedMax = @CurentVal + @StepValue
			end
			else
			begin
				--select @RegularVal = @currdate + right(replicate(0,@length) + cast((@CurentVal + @StepValue) as varchar), @length) 
				select SeedType = @seqname, SeedValue = @PreFixString + @datetime + right(replicate(0,@length) + cast((@CurentVal + @StepValue) as varchar), @length) + @PostFixString, SeedMin = @CurentVal + 1, SeedMax = @CurentVal + @StepValue
			end
			commit tran T2
		end
end
